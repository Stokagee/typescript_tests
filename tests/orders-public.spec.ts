...
test.describe("Public non-auth endpoints", () => {
  ...
  test("GET /api/v1/orders/pending vrací 401 bez autentizace", async ({ request }) => {
  ...
  test("GET /api/v1/orders/by-status/CREATED vrací 401 bez autentizace", async ({ request }) => {
    const response = await request.get(
      "/api/v1/orders/by-status/CREATED",
      {
        failOnStatusCode: false,
      }
    );
